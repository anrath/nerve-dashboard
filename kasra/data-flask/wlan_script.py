...
def wlan_params(name="campus"):
    ...
    params["name_desc"] = int(
        wlan_df[~wlan_df["device_name"].str.contains(":", na=False)]["device_name"].value_counts().sum()
    )
    params["name_nondesc"] = int(
        wlan_df[wlan_df["device_name"].str.contains(":", na=False)]["device_name"].value_counts().sum()
    )
    params["manuf_known"] = int(
        wlan_df[~wlan_df["manuf"].str.contains("Unknown", na=False)]["manuf"].value_counts().sum()
    )
...
def wlan_devicename_dist(wlan_df, file_name="wlan_devicename_dist.svg"):
    ...
    fig = (
        ...
        .plot(kind="pie", legend=True, title="Device Names", labeldistance=None)
    )  # autopct="%1.0f%%"
...
def wlan_manuf_dist(wlan_df, file_name="wlan_manuf_dist.svg"):
    ...
    fig = (
        wlan_df[~wlan_df["manuf"].str.contains("Unknown", na=False)]["manuf"]
        .value_counts()
        .plot(kind="pie", legend=True, title="Device Manufacturers", labeldistance=None)
    )  # autopct="%1.0f%%"
