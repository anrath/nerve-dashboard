...
def net_exist(ap):
    ssid = ""
    try:
        ssid = ap["dot11.device.responded_ssid_map"][0]["dot11.advertisedssid.ssid"]
    except:
        try:
            ssid = ap["dot11.device.advertised_ssid_map"][0]["dot11.advertisedssid.ssid"]
        except:
            try:
                ssid = ap["dot11.device.probed_ssid_map"][0]["dot11.probedssid.ssid"]
            except:
                ssid = "eduroam"
    return ssid
...
def create_ssid_graph(sub_path):
    if sub_path == "realtime":
        ...
        ap_response = requests.get(
            user_password + server_ip + ap_endpoint, headers={"KISMET": "E62F6C667B3CF269798AC58E0D811D85"}
        )
        ap_json = json.dumps(ap_response.json(), indent=4)

        wlan_endpoint = "/devices/views/phy-IEEE802.11/devices.json"
        wlan_response = requests.get(
            user_password + server_ip + wlan_endpoint, headers={"KISMET": "E62F6C667B3CF269798AC58E0D811D85"}
        )
        ...
        ssid_response = requests.get(
            user_password + server_ip + ssid_endpoint, headers={"KISMET": "E62F6C667B3CF269798AC58E0D811D85"}
        )
    ...
    aps_df = pd.read_json(f"{DATA_PATH}/{sub_path}/phydot11_accesspoints.json", orient="records")
    aps_df = aps_df[
        ["kismet.device.base.key", "kismet.device.base.manuf", "kismet.device.base.macaddr", "dot11.device"]
    ]
    aps_df.rename(
        columns={
    ...
    aps_df["ssid"] = aps_df.apply(lambda row: net_exist(row["ssid"]), axis=1)
    ...
    wlan_df = wlan_df[
        ["dot11.device", "kismet.device.base.key", "kismet.device.base.manuf", "kismet.device.base.macaddr"]
    ]
    wlan_df.rename(
        columns={
            "dot11.device": "ap",
    ...
    wlan_df["ap"] = wlan_df.apply(lambda row: row["ap"]["dot11.device.last_bssid"], axis=1)
    ...
    for ssid in ssids:
        ...
        if len(ssid["dot11.ssidgroup.responding_devices"]) != 0:
            ...
            temp_res = pd.merge(
                temp_res, aps_df.loc[:, ~aps_df.columns.isin(["ssid"])], left_on="devices", right_on="key", how="inner"
            )
    ...
    images = {
        "Intel Corporation": "https://raw.githubusercontent.com/anrath/data/master/intel_logo.jpg",
    ...
    for e in edge_data:
        ...
        if manuf in images.keys():
            ...
            net.add_node(
                ...
                group=device_group,
                shape="circularImage",
                image="https://raw.githubusercontent.com/anrath/data/master/blank.png",
                brokenImage="circle",
            )
    ...
    for e in edge_data:
        ...
        net.add_node(
            ...
            group=2,
            shape="circularImage",
            image=images["Aruba Networks HP"],
            brokenImage="circle",
        )
